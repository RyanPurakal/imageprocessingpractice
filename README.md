...
| Track in git | Do not commit (see `.gitignore`) |
|--------------|----------------------------------|
| Source, `README.md`, `requirements.txt`, `scripts/`, `outputs/*.png`, `outputs/metrics.csv` | `.venv/`, `checkpoints/`, `*.pt`, `.mplconfig/`, `__pycache__/` |

Weights are large; regenerate with `./scripts/demo.sh` or `python train.py` after clone.
...
Include **either** regenerated `outputs/*.png` + `metrics.csv` from your demo run **or** a short note to run `./scripts/demo.sh` after unzipping. Do **not** bundle `checkpoints/` or `.venv/` unless you use Git LFS and mean to share weights.
